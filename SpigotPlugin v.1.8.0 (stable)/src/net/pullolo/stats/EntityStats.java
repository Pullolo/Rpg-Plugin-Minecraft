...
import net.pullolo.Entities.DamageIndicator;
...
public class EntityStats {
    ...
    public void dealDamage(Entity e, int amount, boolean isCrit){
        int damage;
        if (isCrit){
            Random rand = new Random();
            if (rand.nextInt() == 1){
                damage = amount * 2;
            } else damage = amount * 5;
        } else {
            damage = amount;
        }
        if (defense+bonusDefense <= 10){
            this.health -= damage;
            DamageIndicator indicator = new DamageIndicator(e, damage, isCrit);
        } else{
            this.health -= damage/((defense+bonusDefense) /10);
            DamageIndicator indicator = new DamageIndicator(e, damage/((defense+bonusDefense) /10), isCrit);
        }
    }
